...
class ExportController:
    ...
    async def export_to_pdf(self, data: Dict[str, Any]) -> StreamingResponse:
        ...
        try:
            ...
            logger.info(f"Analysis data type: {type(data.get('analysis'))}")
            
            # Generate PDF content using enhanced method (same quality as email)
            ...
            logger.error(f"PDF export error type: {type(e)}")
            logger.error(f"PDF export error args: {e.args}")
            import traceback
    ...
    async def generate_enhanced_pdf(self, analysis: DocumentAnalysisResponse) -> bytes:
        ...
        try:
            ...
            logger.info("Enhanced PDF imports successful")
            ...
            # Professional header with shield logo
            ...
            # Shield logo and company branding
            content.append(Paragraph("🛡️", ParagraphStyle(
                'ShieldLogo',
                parent=styles['Normal'],
                fontSize=48,
                alignment=1,
            ...
            content.append(Paragraph("LegalSaathi", title_style))
            content.append(Paragraph("🛡️ Trusted Legal AI Analysis Platform 🛡️", header_style))
            content.append(Paragraph("AI-Powered Legal Document Analysis Report", subtitle_style))
            content.append(Paragraph("Professional Legal Document Assessment & Risk Evaluation", styles['Normal']))
            ...
            for i, clause in enumerate(analysis.clause_assessments):
                ...
                if clause.clause_text and clause.clause_text.strip():
                    content.append(Paragraph("<b>Clause Text:</b>", styles['Normal']))
                    content.append(Paragraph(f'"{clause.clause_text}"', styles['Normal']))
                    content.append(Spacer(1, 8))
                
                # Clause explanation
                content.append(Paragraph("<b>Professional Analysis:</b>", styles['Normal']))
                ...
                if clause.recommendations:
                    content.append(Paragraph("<b>Professional Recommendations:</b>", styles['Normal']))
            ...
            content.append(Paragraph("🛡️ Generated by LegalSaathi AI Analysis System 🛡️", footer_style))
            content.append(Paragraph(f"Report generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", footer_style))
            content.append(Paragraph("🛡️ Protecting Your Legal Interests with AI-Powered Analysis 🛡️", footer_style))
            content.append(Spacer(1, 20))
            ...
            While our analysis utilizes advanced technology and professional review processes, it should not be 
            relied upon as a substitute for consultation with a qualified attorney. For matters requiring legal 
            counsel or professional representation, please consult with a licensed attorney in your jurisdiction. 
            The accuracy and completeness of analysis may vary based on document complexity and other factors.
            """
            
            content.append(Paragraph(disclaimer_text, disclaimer_style))
            
            # Build PDF
            logger.info("Building enhanced PDF document")
            doc.build(content)
            ...
            logger.info(f"Enhanced PDF completed, size: {len(pdf_bytes)} bytes")
            return pdf_bytes
            ...
            raise HTTPException(
                status_code=500,
                detail="PDF generation library not available. Please install ReportLab or contact support."
            )
        except Exception as e:
        ...
        if isinstance(data.get('analysis'), dict):
            try:
                ...
                if 'analysis_id' not in analysis_data:
                    analysis_data['analysis_id'] = f"analysis_{int(datetime.now().timestamp())}"
                if 'processing_time' not in analysis_data:
                    analysis_data['processing_time'] = 0.0
                ...
                logger.error(f"Failed to parse analysis from dict: {e}")
                logger.error(f"Analysis data: {data.get('analysis', {})}")
            ...
            try:
                ...
                logger.error(f"Failed to convert Pydantic model: {e}")
            ...
            try:
                ...
                logger.error(f"Failed to convert object attributes: {e}")
        
        # If we successfully parsed the analysis, use enhanced PDF
        if analysis:
            try:
                logger.info("Using enhanced PDF generation (same as email)")
                logger.info(f"Analysis object created successfully: {analysis.analysis_id}")
                enhanced_pdf = await self.generate_enhanced_pdf(analysis)
                logger.info(f"Enhanced PDF generated successfully, size: {len(enhanced_pdf)} bytes")
                return enhanced_pdf
            except Exception as e:
                logger.error(f"Enhanced PDF generation failed: {e}")
                import traceback
                logger.error(f"Enhanced PDF traceback: {traceback.format_exc()}")
                # Try to use the working simple PDF method instead
                logger.warning("Falling back to simple PDF due to enhanced PDF failure")
                return await self._generate_working_simple_pdf(data)
        
        # If no analysis object could be created, try simple PDF
        logger.warning("Could not create DocumentAnalysisResponse, using simple PDF")
        logger.info(f"Data passed to simple PDF: {data}")
        return await self._generate_working_simple_pdf(data)
    
    async def _generate_test_pdf(self) -> bytes:
        """Generate a very simple test PDF to verify ReportLab is working"""
        try:
            ...
            logger.info(f"Test PDF generated successfully, size: {len(pdf_bytes)} bytes")
            return pdf_bytes
    ...
    async def _generate_working_simple_pdf(self, data: Dict[str, Any]) -> bytes:
        """Generate a working simple PDF that always returns valid PDF bytes"""
        try:
            logger.info("Starting working simple PDF generation")
            from reportlab.lib.pagesizes import A4
            ...
            # Add title
            content.append(Paragraph("🏛️ LegalSaathi", styles['Title']))
            content.append(Paragraph("Legal Document Analysis Report", styles['Heading1']))
            content.append(Spacer(1, 20))
            ...
            if analysis:
                ...
                if overall_risk:
                    content.append(Paragraph(f"Overall Risk Level: {overall_risk.get('level', 'Unknown')}", styles['Normal']))
                    content.append(Paragraph(f"Risk Score: {overall_risk.get('score', 0):.1%}", styles['Normal']))
                    content.append(Paragraph(f"Confidence: {overall_risk.get('confidence_percentage', 0)}%", styles['Normal']))
                    content.append(Spacer(1, 15))
                ...
                content.append(Paragraph("Analysis data not available", styles['Normal']))
            
            # Add footer
            content.append(Spacer(1, 30))
            content.append(Paragraph("🛡️ Generated by LegalSaathi AI Analysis System 🛡️", styles['Normal']))
            ...
            logger.info(f"Working simple PDF generated, size: {len(pdf_bytes)} bytes")
            return pdf_bytes
            
        except Exception as e:
            logger.error(f"Working simple PDF generation failed: {e}")
            logger.error(f"Simple PDF error type: {type(e)}")
            import traceback
            logger.error(f"Simple PDF traceback: {traceback.format_exc()}")
            # Create minimal PDF using only basic ReportLab
            return await self._generate_minimal_pdf()
    
    async def _generate_minimal_pdf(self) -> bytes:
        """Generate the most basic PDF possible"""
        try:
            logger.info("Starting minimal PDF generation")
            from reportlab.pdfgen import canvas
            from reportlab.lib.pagesizes import A4
            logger.info("Minimal PDF imports successful")
            ...
            p.drawString(100, 750, "LegalSaathi Document Analysis Report")
            
            # Add date
            p.setFont("Helvetica", 12)
            p.drawString(100, 720, f"Generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}")
            
            # Add content
            p.drawString(100, 680, "Your document analysis is complete.")
            p.drawString(100, 660, "Please contact support if you need detailed results.")
            
            # Add footer
            p.drawString(100, 100, "🛡️ Generated by LegalSaathi AI Analysis System 🛡️")
            
            p.save()
            pdf_bytes = buffer.getvalue()
            buffer.close()
            
            logger.info(f"Minimal PDF generated, size: {len(pdf_bytes)} bytes")
            return pdf_bytes
            
        except Exception as e:
            logger.error(f"Minimal PDF generation failed: {e}")
            raise HTTPException(
                status_code=500,
                detail="PDF generation completely failed. Please contact support."
            )
    ...
    async def _generate_simple_pdf_content(self, data: Dict[str, Any]) -> bytes:
        ...
        try:
            ...
            # Add branded title
            content.append(Paragraph("LegalSaathi", title_style))
            content.append(Paragraph("Professional Legal Document Analysis Report", styles['Heading2']))
            content.append(Spacer(1, 30))
            ...
            content.append(Paragraph("Generated by LegalSaathi Professional Analysis System", footer_style))
            content.append(Paragraph(f"Report generated on {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", footer_style))
