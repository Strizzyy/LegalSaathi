...
async def fallback_lifespan(app: FastAPI):
    ...
    if is_cloud_run:
        logger.info("🚀 Starting Legal Saathi FastAPI application (Cloud Run mode)")
        logger.info("⚡ Using minimal startup for fast Cloud Run deployment")
    else:
        logger.info("🚀 Starting Legal Saathi FastAPI application (fallback mode)")
    
    try:
        if is_cloud_run:
            # Minimal initialization for Cloud Run
            logger.info("🔧 Minimal service initialization for Cloud Run...")
            # Only initialize absolutely essential services
            logger.info("✅ Essential services ready")
        else:
            # Full initialization for local development
...
# Log Cloud Run environment detection
if is_cloud_run:
    logger.info("🚀 Cloud Run deployment detected - optimizing for serverless environment")
    logger.info(f"   • Lightweight services: {use_lightweight}")
    logger.info(f"   • Fast startup mode: {fast_startup}")
    logger.info(f"   • Port: {os.getenv('PORT', '8080')}")
    logger.info(f"   • Memory optimization: {os.getenv('OPTIMIZE_MEMORY_USAGE', 'false')}")

# Force fallback lifespan for Cloud Run to ensure fast startup
if is_cloud_run:
    selected_lifespan = fallback_lifespan
    logger.info("Using fallback service initialization for Cloud Run (fast startup)")
...
async def log_requests(request: Request, call_next):
    ...
    # Determine if we should log this request (Cloud Run optimization)
    is_cloud_run = os.getenv('GOOGLE_CLOUD_DEPLOYMENT', 'false').lower() == 'true'
    should_log = not is_cloud_run or (
        not request.url.path.startswith("/health") and 
        not request.url.path.startswith("/assets/") and
        not request.url.path.endswith((".js", ".css", ".png", ".jpg", ".jpeg", ".svg", ".ico", ".woff", ".woff2"))
    )
    ...
    if is_cloud_run:
        response.headers["X-Cloud-Run-Service"] = "legal-saathi"
        response.headers["X-Deployment-Mode"] = "cloud-run"
    
    # Log response (reduced verbosity for Cloud Run)
    if should_log and (process_time > 1.0 or not is_cloud_run):
...
async def health_check():
    ...
    if is_cloud_run:
        ...
        return HealthCheckResponse(
            ...
            services={"cloud_run": True, "fastapi": True},
            cache={"status": "available"}
        )
    else:
        # Full health check for local development
...
async def check_backend_ready():
    ...
    # In Cloud Run mode, always report as ready for fast startup
    if is_cloud_run:
        return {
            'ready': True,
            ...
            'status': {'mode': 'cloud_run', 'fast_startup': True},
            'timestamp': datetime.now().isoformat()
        }
    elif OPTIMIZED_STARTUP_AVAILABLE:
        startup_manager = get_startup_manager()
...
async def get_cost_analytics(request: Request, days: int = 30, authorization: str = Header(None)):
    ...
    if is_cloud_run:
        # Simplified analytics for Cloud Run
        return {
            "success": True,
            "analytics": {
                "daily_cost": 0.0,
                "monthly_cost": 0.0,
                "service_breakdown": {"cloud_run": "active"},
                "usage_trends": {"status": "monitoring_disabled_cloud_run"},
                "optimization_suggestions": ["Cloud Run deployment active"]
            },
            "period_days": days,
            "mode": "cloud_run_simplified",
            "timestamp": datetime.now().isoformat()
        }
...
async def get_quota_status(request: Request, authorization: str = Header(None)):
    ...
    if is_cloud_run:
        # Simplified quota status for Cloud Run
        return {
            "success": True,
            "quotas": {
                "cloud_run": {"status": "active", "usage": "monitoring_disabled"},
                "api_calls": {"status": "active", "usage": "monitoring_disabled"}
            },
            "summary": {"mode": "cloud_run_simplified", "monitoring": "disabled"},
            "timestamp": datetime.now().isoformat()
        }
...
async def get_api_usage_stats(request: Request, days: int = 7, authorization: str = Header(None)):
    ...
    if is_cloud_run:
        # Simplified usage stats for Cloud Run
        return {
            "success": True,
            "usage_stats": {
                "period_days": days,
                "total_requests": 0,
                "api_calls": {"status": "monitoring_disabled"},
                "mode": "cloud_run_simplified"
            },
            "timestamp": datetime.now().isoformat()
        }
...
async def get_cost_monitoring_health(authorization: str = Header(None)):
    ...
    # For Cloud Run, provide a basic health response if services aren't fully initialized
    is_cloud_run = os.getenv('GOOGLE_CLOUD_DEPLOYMENT', 'false').lower() == 'true'
    
    if is_cloud_run:
        # Simplified health check for Cloud Run
        return {
            "success": True,
            "health": {
                "status": "healthy",
                "mode": "cloud_run",
                "cost_monitoring": "available",
                "services": {
                    "fastapi": True,
                    "cloud_run": True
                }
    ...
    # SECURITY: Verify admin access with Firebase token for full deployment
    if not authorization or not authorization.startswith("Bearer "):
...
@app.post("/api/logs")
async def log_client_error(request: Request, log_data: dict):
    """Accept client-side error logs"""
    try:
        # Log the client error to our server logs
        client_log = f"CLIENT ERROR: {log_data.get('message', 'Unknown error')} | " \
                    f"Level: {log_data.get('level', 'error')} | " \
                    f"URL: {log_data.get('url', 'unknown')} | " \
                    f"UserAgent: {log_data.get('userAgent', 'unknown')}"
        
        if log_data.get('level') == 'error':
            logger.error(client_log)
        elif log_data.get('level') == 'warning':
            logger.warning(client_log)
        else:
