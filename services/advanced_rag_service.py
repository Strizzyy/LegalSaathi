...
# Conditional imports for Cloud Run compatibility
try:
...
class AdvancedRAGService:
    ...
    def __init__(self):
        ...
        self.disable_heavy_models = (
            ...
            self.is_cloud_run or self.use_lightweight_services or
            not SENTENCE_TRANSFORMERS_AVAILABLE or not FAISS_AVAILABLE
        )
    ...
    def _initialize_lightweight_models(self):
        """Initialize lightweight models for Cloud Run deployment"""
        logger.info("🚀 Initializing lightweight models for Cloud Run")
        
        # Use the lightweight RAG service instead of heavy models
        try:
            from services.lightweight_rag_service import lightweight_rag_service
            self.lightweight_service = lightweight_rag_service
            logger.info("✅ Lightweight RAG service loaded successfully")
        except ImportError as e:
            logger.warning(f"Lightweight RAG service not available: {e}")
            self.lightweight_service = None
        
        # Set all heavy models to None
        self.embedding_model = None
        self.cross_encoder = None
        self.vector_store = None
        self.bm25_retriever = None
        
        logger.info("✅ Lightweight models initialized for Cloud Run")
    ...
    async def _build_bm25_index(self, chunks: List[str]):
        ...
        if BM25_AVAILABLE:
            ...
            logger.warning("⚠️ BM25 not available - skipping sparse retrieval index")
            self.bm25_retriever = None
...
# Global instance - with Cloud Run error handling
try:
    ...
    advanced_rag_service = FallbackRAGService()